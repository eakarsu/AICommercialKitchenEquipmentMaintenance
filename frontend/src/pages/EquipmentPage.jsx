...
const statusBadgeColors = {
  operational: 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30',
  needs_maintenance: 'bg-amber-500/15 text-amber-400 border border-amber-500/30',
  out_of_service: 'bg-red-500/15 text-red-400 border border-red-500/30',
};
...
function StatusBadge({ status }) {
  ...
  const colors = statusBadgeColors[status] || 'bg-slate-500/15 text-slate-400 border border-slate-500/30';
  return (
...
export default function EquipmentPage() {
  ...
  const handleAI = async (item) => {
    try {
      setAiLoading(true);
      setAiResult(null);
      const result = await api.aiAnalyzeEquipment(item.id || item._id);
      setAiResult(result);
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    try {
      if (editItem) {
        await api.updateEquipment(editItem.id || editItem._id, data);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Manage and monitor all kitchen equipment
          </p>
        </div>
        <button
        ...
        {!loading && filtered.length > 0 && (
          <div className="border-t border-slate-700/50 px-5 py-3 flex items-center justify-between bg-slate-900/50">
            <p className="text-xs text-slate-500">
              Showing {filtered.length} of {items.length} equipment
            </p>
          </div>
        )}
      </div>
