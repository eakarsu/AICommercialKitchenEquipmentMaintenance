...
export default function Gap0MountedChatStyleAiEndpointsDespiteAicenterPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-0-mounted-chat-style-ai-endpoints-despite-aicenter/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>0 mounted /chat-style AI endpoints despite aiCenter.js / aiNew.js scaffolds</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-0-mounted-chat-style-ai-endpoints-despite-aicenter/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
