...
export default function AgenticFieldServiceDispatcherRoutingJobsBySPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-field-service-dispatcher-routing-jobs-by-s/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic field-service dispatcher routing jobs by SLA, parts on truck, and skill</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-field-service-dispatcher-routing-jobs-by-s/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
