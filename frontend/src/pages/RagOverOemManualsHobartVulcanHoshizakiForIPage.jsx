...
export default function RagOverOemManualsHobartVulcanHoshizakiForIPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-rag-over-oem-manuals-hobart-vulcan-hoshizaki-for-i/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over OEM manuals (Hobart, Vulcan, Hoshizaki) for instant troubleshooting</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-oem-manuals-hobart-vulcan-hoshizaki-for-i/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
