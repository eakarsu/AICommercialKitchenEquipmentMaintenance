...
export default function RealTimeIotTelemetryFromKitchenEquipmentWitPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-real-time-iot-telemetry-from-kitchen-equipment-wit/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time IoT telemetry from kitchen equipment with proactive repair tickets</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-iot-telemetry-from-kitchen-equipment-wit/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
