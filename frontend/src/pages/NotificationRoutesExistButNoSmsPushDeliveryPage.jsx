...
export default function NotificationRoutesExistButNoSmsPushDeliveryPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-notification-routes-exist-but-no-sms-push-delivery/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Notification routes exist but no SMS/push delivery channel</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-notification-routes-exist-but-no-sms-push-delivery/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
